...
int main(int argc, char *argv[]) {
    ...
    int write_coal_time_matrix(hgt_pop **ps, hgt_params *params, int rank, int numprocs);
...
int write_coal_time_matrix(hgt_pop **ps, hgt_params *params, int rank, int numprocs) {
    FILE *fp;
    if (rank == 0) {
        fp = create_file(params->prefix, "coal_times", "json");
    }

    double **matrix;
    matrix = (double **) malloc(params->size * sizeof(double*));
    int i;
    for (i = 0; i < params->size; i++) {
        matrix[i] = (double *)malloc(params->size * sizeof(double));
    }
