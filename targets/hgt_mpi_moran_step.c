...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < params->sample_time; i++) {
        start = clock();
        hgt_utils_batch_evolve(ps, params->replicates, params, hgt_pop_sample_moran, hgt_pop_coal_time_moran, rr[0]);
        corr_calc(p2means, p2vars, pxy, d1, d2, ps, params, rank, numprocs, hgt_cov_sample_p2, rr);
        corr_calc(p3means, p3vars, pxy, d1, d2, ps, params, rank, numprocs, hgt_cov_sample_p3, rr);
        corr_calc(p4means, p4vars, pxy, d1, d2, ps, params, rank, numprocs, hgt_cov_sample_p4, rr);
...
int corr_calc(hgt_stat_mean ***means, hgt_stat_variance ***vars, double *pxy, double *d1, double *d2,hgt_pop **ps, hgt_pop_params *params, int rank, int numprocs, hgt_cov_sample_func sample_func, gsl_rng **rr) {
    ...
    for (i = 0; i < params->replicates; i++) {
        for (n = 0; n < params->sample_size; n++) {
            hgt_pop_calc_dist(ps[i], d1, d2, 1, sample_func, rr[i]);
