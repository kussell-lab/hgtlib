...
int hgt_pop_mutate_step(hgt_pop *p, hgt_params *params, const gsl_rng *r) {
    unsigned g;
    // randomly choose a cell.
    g = gsl_rng_uniform_int(r, p->size);
    hgt_genome_fitness_mutate(p->genomes[g], 0, params->fitness_scale);
...
int hgt_pop_transfer(hgt_pop *p, hgt_params* params, unsigned long frag_len, const gsl_rng* r) {
...
int update_linkages_wf(hgt_pop *p, int *offsprings) {
    int linkage_linear_selection(hgt_linkage ** current_linkages, int * offsprings, int current_size, hgt_linkage ** new_linkages, unsigned long generation);
